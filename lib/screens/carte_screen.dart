...
class _CartePageState extends State<CartePage> {
  ...
  Future<void> _chargerZonesDanger() async {
    ...
    final List<Future<ZoneDanger>> futures = service.getZones().map((
      zoneSecu,
    ) async {
      double score = await service.getScoreForPosition(
        zoneSecu.latitude,
        zoneSecu.longitude,
      );
      int niv;
      ...
      return ZoneDanger(
        LatLng(zoneSecu.latitude, zoneSecu.longitude),
        20000,
        niv,
      );
    }).toList();
  ...
  Future<void> _appliquerSelectionLieu(
    LatLng coordonnes,
    String lieu,
    bool estDepart,
  ) async {
    ...
    if (ItineraireServices.depart != null &&
      ...
      child: Stack(
        children: [
          carte(
            ...
            ItineraireServices.arrivee,
            ItineraireServices
                    .trajetsParMode[ItineraireServices.modeActuel]
                    ?.trajet ??
                [],
            zonesDanger,
            _mapController,
          ...
          barreModeTransport(
            ItineraireServices.modeActuel,
            _onModeTransportChange,
          ),
    ...
    return Column(
      children: [
        Typeahead(
          ...
          _departController,
          onSelected: (coordonnees, lieu) =>
              _appliquerSelectionLieu(coordonnees, lieu, true),
        ...
        Typeahead(
          ...
          _arriveeController,
          onSelected: (coordonnees, lieu) =>
              _appliquerSelectionLieu(coordonnees, lieu, false),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Column(children: [_buildBarreRecherche(), _buildZoneCarte()]),
      ),
    );
  }
}
