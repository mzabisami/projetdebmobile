...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _buildStatsRow(Map<String, dynamic> stats) {
    ...
    return Row(
      children: [
        _buildStatCard(
          icon: Icons.eco, label: 'Trajets verts', value: '$ecoRoutes',
          bgColor: const Color(0xFFD1FAE5), iconColor: _emeraldStart,
        ),
        const SizedBox(width: 12),
        _buildStatCard(
          icon: Icons.emoji_events, label: 'Points', value: '$totalPoints',
        ...
        _buildStatCard(
          ...
          bgColor: const Color(0xFFFEF3C7), iconColor: _warningOrange,
        ),
      ],
    );
  }
  ...
  Widget _buildPointsBreakdown(Map<String, dynamic> stats) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Répartition des points',
              style: TextStyle(color: Colors.white,
                  fontSize: 17, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _buildPointsSubCard(
              icon: Icons.eco, label: 'Points Éco', value: '$ecoPoints',
              sub: totalPts > 0 ? '${(ecoPoints / totalPts * 100).round()}% du total' : '0%',
            )),
            const SizedBox(width: 12),
            Expanded(child: _buildPointsSubCard(
              icon: Icons.shield, label: 'Points Sécurité', value: '$secPts',
              sub: totalPts > 0 ? '${(secPts / totalPts * 100).round()}% du total' : '0%',
            )),
          ]),
          const SizedBox(height: 12),
          const Text(
      ...
      children: [
        ...
        Row(children: [
          Expanded(child: _buildResultCard(
            icon: Icons.trending_down, label: 'CO₂ économisé',
            value: '${(weekCo2 / 1000).toStringAsFixed(1)} kg',
            sub: 'Cette semaine',
            gradient: const LinearGradient(colors: [_emeraldStart, _emeraldEnd]),
          )),
          const SizedBox(width: 12),
          Expanded(child: _buildResultCard(
            icon: Icons.emoji_events, label: 'Points gagnés',
            value: '$weekPoints',
            sub: 'Cette semaine',
            gradient: const LinearGradient(colors: [_blueStart, _indigoEnd]),
          )),
        ]),
      ],
    );
          ...
          Row(
            ...
            children: const [
              ...
              Row(children: [
                ...
                SizedBox(width: 4),
                Text('+12%', style: TextStyle(color: _emeraldStart, fontSize: 12)),
              ]),
            ],
          ),
          const SizedBox(height: 16),
          ...
          Row(
            ...
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Trajets écologiques',
                    style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                Text('Sur $totalRoutes Trajets totaux',
  ...
  Widget _buildModeBreakdown(Map<String, int> m) {
    return Row(children: [
      _modeItem('vélo',    m['vélo']    ?? 0, _emeraldStart, const Color(0xFFD1FAE5), '🚲'),
      _modeItem('bus',     m['bus']     ?? 0, _blueStart,    const Color(0xFFDBEAFE), '🚌'),
      _modeItem('métro',   m['métro']   ?? 0, _purpleColor,  const Color(0xFFF3E8FF), '🚇'),
      _modeItem('voiture', m['voiture'] ?? 0, _dangerRed,    const Color(0xFFFEE2E2), '🚗'),
    ]);
  }

  Widget _modeItem(
      String mode, int count, Color color, Color bgColor, String emoji) {
    return Expanded(
      child: Container(
  ...
  Widget _buildGoals(Map<String, dynamic> stats) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Objectifs du mois',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 12),
          _buildGoalBar(
            label: 'Économiser 50 kg de CO₂',
            current: co2Kg, target: 50, color: _emeraldStart,
          ),
          const SizedBox(height: 12),
          _buildGoalBar(
            label: 'Faire 60 Trajets verts',
            current: ecoRoutes, target: 60, color: _blueStart,
          ),
        ],
      ),
    );
  ...
  Widget _buildGoalBar({
    required String label, required int current,
    required int target,   required Color color,
  }) {
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Column(children: [
            _buildSettingsItem(Icons.person_outline, 'Modifier le profil',
                null, null),
            _buildDivider(),
            _buildSettingsItem(Icons.notifications_outlined, 'Notifications',
                null, null),
            _buildDivider(),
            _buildSettingsItem(Icons.settings_outlined, 'Préférences',
                null, null),
            _buildDivider(),
            _buildSettingsItem(Icons.help_outline, 'Aide & FAQ',
                _blueStart, const Color(0xFFDBEAFE)),
          ]),
        ),
      ],
    );
  ...
  Widget _buildSettingsItem(
      IconData icon, String label, Color? iconColor, Color? iconBg) {
    return ListTile(
      leading: Container(
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(
          color: iconBg ?? Colors.grey[100], shape: BoxShape.circle,
        ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Votre impact écologique',
                    style: TextStyle(
