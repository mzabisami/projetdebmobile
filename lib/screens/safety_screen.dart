...
class _SafetyScreenState extends State<SafetyScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final double avg = routes.isEmpty ? 0.0
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Vos Trajets sont analysés en temps réel',
            ...
            child: Row(
              ...
              children: [
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Score moyen',
                      style: TextStyle(color: Colors.white70, fontSize: 13)),
                  Text('${_avgScore.toStringAsFixed(0)}/100',
  ...
  Widget _buildCriteriaCard() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Critères de sécurité',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 16),
          _buildCriterionBar(
            icon: Icons.wb_sunny, label: 'Éclairage',
            value: _avgLighting, color: _yellowColor,
          ),
          const SizedBox(height: 12),
          _buildCriterionBar(
            icon: Icons.directions_bike, label: 'Pistes cyclables',
            value: _avgCycling, color: _emeraldStart,
          ),
          const SizedBox(height: 12),
          _buildCriterionBar(
            icon: Icons.navigation, label: 'Fluidité du trafic',
            value: (1 - _avgTraffic).clamp(0.0, 1.0),
            color: _blueStart,
          ),
          const SizedBox(height: 12),
          _buildCriterionBar(
            icon: Icons.people, label: 'Fréquentation',
            value: _avgCrowding, color: _purpleColor,
          ),
        ],
      ),
    );
  ...
  Widget _buildRecentRoutes() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Vos Trajets récents',
        ...
        if (_recentRoutes.isEmpty)
          Container(
            ...
            child: const Center(
              child: Text('Aucun trajet pour le moment 🚶',
  ...
  Widget _buildRouteCard(Trajets route) {
    final Color statusColor = route.securityScore >= 90 ? _emeraldStart
        : route.securityScore >= 75 ? _blueStart : _orangeColor;
    final Color statusBg = route.securityScore >= 90 ? const Color(0xFFD1FAE5)
        : route.securityScore >= 75 ? const Color(0xFFDBEAFE) : const Color(0xFFFFEDD5);
    return Container(
      ...
      child: Row(children: [
        ...
        Column(children: [
          Container(
            ...
            child: Text(route.securityScore.toStringAsFixed(0),
          ...
          Row(children: [
            ...
            Text('+${_ptsForScore(route.securityScore)} pts',
                style: const TextStyle(
                    color: _blueStart, fontSize: 11, fontWeight: FontWeight.bold)),
          ]),
        ]),
      ]),
    );
  ...
  Widget _buildSafetyTips() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Conseils de sécurité',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
        const SizedBox(height: 10),
        _buildTipCard(Icons.wb_sunny, 'Éclairage optimal',
            'Privilégiez les rues bien éclairées le soir', '+5 pts'),
        _buildTipCard(Icons.directions_bike, 'Pistes cyclables',
            'Utilisez les pistes dédiées pour plus de sécurité', '+8 pts'),
        _buildTipCard(Icons.people, 'Zones fréquentées',
            'Les zones avec plus de passage sont plus sûres', '+3 pts'),
      ],
    );
  }

  Widget _buildTipCard(
      IconData icon, String title, String desc, String bonus) {
    return Container(
      ...
      child: Row(children: [
        Container(
          padding: const EdgeInsets.all(8),
          decoration: BoxDecoration(
            color: _blueStart, borderRadius: BorderRadius.circular(10),
          ),
