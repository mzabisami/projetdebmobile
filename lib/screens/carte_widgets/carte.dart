  ...
  return FlutterMap(
    ...
    options: MapOptions(
      ...
      onLongPress: (tapPosition, point) {
        ...
        ); // Permet de réinitialiser le focus sur la carte après un tap
      },
    ),
    children: [
      TileLayer(
    ...
    circles: [
      for (int i = 0; i < zonesDanger.length; i++)
        CircleMarker(
          ...
          color: zonesDanger[i].niveau == 1
              ? Colors.red.withValues(alpha: 0.5)
              : zonesDanger[i].niveau == 2
...
PolylineLayer _layerTrajet(List<LatLng>? trajet) {
  if (trajet != null && trajet.isNotEmpty) {
    return PolylineLayer(
      polylines: [
        Polyline(points: trajet, strokeWidth: 4.0, color: Colors.blue),
      ],
    );
  } else {
